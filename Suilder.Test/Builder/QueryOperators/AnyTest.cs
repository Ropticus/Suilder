...
namespace Suilder.Test.Builder.QueryOperators
{
    public class AnyTest : BuilderBaseTest
    {
        ...
        {
            Person person = null;
            IOperator op = sql.Op(() => person.Id > SqlExp.Any<int>(sql.RawQuery("Subquery")));
        ...
        {
            Exception ex = Assert.Throws<NotSupportedException>(() => SqlExp.Any(sql.RawQuery("Subquery")));
        ...
        {
            Exception ex = Assert.Throws<NotSupportedException>(() => SqlExp.Any<int>(sql.RawQuery("Subquery")));
            Assert.Equal("Only for expressions.", ex.Message);
        }
        ...
        {
            engine.AddOperator(OperatorName.Any, "TRANSLATED");
        ...
        {
            engine.AddOperator(OperatorName.Any, "TRANSLATED", true);
